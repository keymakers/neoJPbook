...
=== インフォーメーション型NEP
インフォーメーション（Information）型NEPは、NEOを改善する新しいアイデアを提案するわけではありませんが、
NEOの既存デザインの詳細についてまとめていたり、NEOのユーザーとコミュニティに対して、一般的なガイドラインを提供しています。
...
さらにメタ型NEPには、次のような性質があります。

  * スタンダードトラック型NEPと違い、NEOのプロトコル自体を超える領域に対応している
  * インフォーメーション型NEPとは違い、変更内容の提案がNEOのコードベースに対してではなく、コミュニティの合意が求められる場合が多い
  * ユーザーには基本的にそのアドバイスに従う義務がある

== NEPの出し方

=== アイデア
NEPを出すプロセスは、NEOを改善する新しいアイデアを思いつくところから始まりますが、
この際に次のようなことが意識されなければなりません。

  * NEPには、キーとなる提案や新しいアイデアがひとつ含まれている必要があること
  * NEPが必要とされるのは、単一のクライアントではなく複数のクライアントに影響を及ぼしたり、複数のアプリケーションで利用される標準を定義するような変更の場合であること

NEPの編集者（エディター）は、焦点が絞りきれていない提案を拒否する権利をもっており、
NEPの提案内容が疑わしい場合は、さらに焦点を絞ったいくつかのNEPに分割することが好ましいです。

=== 執筆者
NEPには、執筆者（チャンピョン）が必要です。
執筆者とは、後述するスタイルとフォーマットに沿ってNEPをデザインし、
公開されている適切な議論を追ってアイデアの合意を形成しようとする人のことをいいます。
...
=== プルリクエスト
NEPの執筆者は、アイデアの有用性をNEOコミュニティから認められた後に、草稿状態のNEPをプルリクエストとして提出することが求められます。
こうすることで、執筆者は草稿状態のNEPを断続的に編集でき、
追加で外部から寄せられるコメントなど、アイデアへのフィードバックを活用できます。

=== 編集者の承認
NEPの執筆に協力する編集者がプルリクエストを承認すれば、
執筆者はそのNEPに番号をつけ、スタンダードトラック型・インフォーメーション型・メタ型のどれかにラベリングし、
「Draft（草稿状態）」とステータスを決めた後に、新しいNEPとしてgitのレポジトリに追加することができます。
NEPの編集者が理由なしにNEPを否定することはないので、
NEPのステータスが否定された場合は、次のような理由が当てはまっていないかを検討してみましょう。
...
アイデアの実装方法は、堅実なものでありプロトコルを複雑化しすぎないことが必要不可欠です。

=== 承認とステータス
NEPは承認されるかどうかやその進捗具合によってステータスが変わります。
...
ここでは次のような主要なステータスを取り上げます。
  ...
  * Replaced（置き換えられた状態）
  ** 新しいNEPに置き換えられた古いNEPにつくステータスです。NEPは既存のNEPに依拠したり置き換えることもできます。
...
ただし、「Motivation（動機付け）」についてはオプション（選択可能）です。
  ...
  * Motivation
  ** NEOのプロトコルの変更を提案するNEPの場合に、新しいNEPが解決できる課題をなぜ既存のプロトコルでは解決できないのかを説明し、適切な動機を存在を記載します。

  * Specification（仕様）
  ** 構文や意味など、新しい変更や機能の技術仕様を記載します。

  * Rationale（根拠）
  ** アイデアの設計や提案内容の根拠を記載します。NEOのコミュニティの合意形成のためにも重要なので、議論の過程で生じた反論も考慮されるのが好ましいです。

  * Backwards Compatibility
  ** 下位互換性の課題とその対処方法について記載します。課題へ対処するために十分な論文を参照することが重要です。
...
==== Preamble（前文）
ここからは、補足としてNEPを出すために記載が必要な前文で守るべきフォーマットとテンプレートについて説明していきます。
...
これは前文のテンプレートです。
RFC822スタイルのヘッダーで情報が記載する必要があります。
*印のついたヘッダーはオプションですが、その他のヘッダーは記載が必須です。
...
=== NEP5
==== NEP5の概要
NEP5は、トークン化されたスマートコントラクトが統一的に対応する仕組みを可能にするための、NEOのブロックチェーンのトークン規格について概説しています。
