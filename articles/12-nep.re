= NEP
#@#担当者:@ 南口
#@# 現時点の文字数は、5327文字です。4500文字前後になるように無駄を省き、内容を調整するつもりです。
...
そして、以降では、NEPを出す際の前文で必要なフォーマットとテンプレートについて説明していきます。
...
=== NEP5
==== NEP5の概要
NEP5では、NEOのブロックチェーンによって、トークン化されたスマートコントラクトが統一的に対応する仕組みが提供されるためのトークン規格について概説されています。

==== NEP5の動機
NEOのブロックチェーンが拡大するにつれて、スマートコントラクトの配備と発動はますます重要になってきます。
しかし、各コントラクトが互いに対応する規格がなければ、それぞれが互いに類似性を持っているにもかかわらず、それぞれ特有のAPIに対するメンテナンスが行われることが求められます。
トークン化されたコントラクトが動作する基本的な仕組みは全て同じなので、それ自体がこの必要のよい実例となります。
そこで、トークンに対応する基本的な方法があれば、全体の仕組みの中で、トークンを利用するスマートコントラクト全てに求められる基本的な動作のメンテナンスを行わずに済みます。

=== NEP6
==== NEP6の概要
NEP6は、NEOが実装された環境間でウォレットファイルがシェア（共有）されるためのウォレット規格について説明されています。

==== NEP6の動機
現在、異なるクライアントのプログラムが異なるウォレットファイルを生成しています。そのため、これらのファイルの形式と保存方法、暗号化方法は全て異なっています。
ユーザーにとっては、ウォレットファイルの形式が異なっていることによって、異なるクライアントのプログラム間を移動することが難しく、もしもプライベートキーを生成することで移動できたとしても、複数のキーをもつ種々のウォレットは処理し切れません。
