...
楕円曲線及び楕円曲線を用いた暗号化にはいくつかのパラメータが存在し、Secp256k1はそれらの各パラメーター（a=0, b=7）が決まっています。
...
K = k * G
//}
ベースポイントのGはsecp256k1標準で決められており、NEOでの全ての公開鍵に対して常に同じです。
...
公開鍵が生成されました。もし、これがSecp256k1で定義された楕円曲線上にない場合、エラーが帰ってくるはずなので、正しい公開鍵だと確認できます。
