...
//indepimage[equation1][楕円曲線を表した数式][scale=0.4]

このa,bの値を変えることで、楕円曲線はさまざまなな曲線の形状になります。
楕円曲線及び楕円曲線を用いた暗号化にはいくつかのパラメータが存在し、たとえば、Bitcoinで使用されているSecp256k1では、それらの各パラメーター（a=0, b=7）が決まっています。
#@# //indepimage[Secp256k1][Secp256k1][scale=0.7]
//image[Secp256k1][Secp256k1]{
//}
注意:これは、実数部分におけるSecp256k1のグラフですが、実際は体Zp上で定義されるランダムに散らばった小さい点です。

一方、NEOでは、楕円曲線の中でもSecp256r1と呼ばれる楕円曲線を利用しています。
これは、各パラメーターが次のようになっており、先ほどのSecp256k1の楕円曲線と比べて複雑であることが分かると思います。

a= -3@<br>{}
b= 41058363725152142129326129780047268409114441015993725554835256314039467401291@<br>{}

注意：パラメータが異なるので、Secp256r1のグラフは、@<img>{Secp256k1}とまったく同じではありません。
...
===[column] 楕円曲線のパラメータのコード内での表現方法
@<hd>{楕円曲線とは}では、bのパラメータが@<code>{41058363725152142129326129780047268409114441015993725554835256314039467401291}という数字であるのにもかかわらず、コードで使用されているのは、@<code>{5AC635D8AA3A93E7B3EBBD55769886BC651D06B0CC53B0F63BCE3C3E27D2604B}という文字列になります。
実は、両者は同じであり、コード内の@<code>{5A~}から始まる文字列を次のサイトで変換したら、@<hd>{楕円曲線とは}であげているbの値と一致することがわかります。

@<href>{https://www.browserling.com/tools/hex-to-dec}

また、aのパラメータは、次のmodの計算により-3という値が算出されています。
//indepimage[equation2][パラメータaとpの関係式]
