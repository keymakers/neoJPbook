...
トランザクションやアセットなど、ネットワーク全体のデータはブロックチェーンに格納されます。ブロックチェーンに格納されているブロックは次のような構造を持ちます。
...
ブロックのハッシュ値を計算するとき、ブロック全体を計算するのではなく、Version, PrevBlock, MerkleRoot, Timestamp, Height, Nonce, NextMinerの7つの値のみ計算されます。MerkleRootには全てのトランザクションのハッシュ値が含まれており、トランザクションの変更はそのブロックのハッシュの値に影響します。
...
NEOではコインの管理方法として、ビットコインなどでも使用されているUTXO（Unspent Transaction Output）という仕組みを利用しています。UTXOとは、ブロックチェーン上でまだ使用されていないトランザクションのアウトプットであり、残高を示します。簡単にいうと、取り引きデータから残高を算出する方法になります。
...
MinerTransactionは、各ブロックのTransactionsのリストの最初に格納され、そのブロックの全ての手数料をバリデーターの報酬にします。Nonce（uintΩ32）をもち、Nonceの値はハッシュ値の衝突を避けるのに使用します。
...
アセットの管理者は、IssueTransactionによってブロックチェーンに登録されているアセットを作成し、任意のアドレスへ送ることができます。NEOから発行されているアセットの場合、トランザクションの手数料はかかりません。IssueTransactionもNonceの値を持ちます。
...
EnrollmentTransaction型のトランザクションを作成し、PublicKeyのアドレスにデポジットを送付することで登録が行われます。ばりデーター候補者の登録をキャンセルする場合は、そのデポジットの残高を0にしてください。
...
AssetTypeには次の値が格納されます。それぞれのアセットには固有の制限があり、NEOおよびAntCoinはシステムの組み込みアセットのためGenesisブロック（高さが0のブロック）でのみ作成することができます。equityのようなアセットを作成するときは、アセットの合計額を制限し、送信者と受信者の両方がトランザクションに署名しなくてはなりません。Currencyのアセット作成時には、アセットの合計額は制限することができません。
...
ContractTransactionは、アセットを転送するためのトランザクションです。NEOやGASなどのアセットを転送することを目的としており、ブロックチェーンでもっとも一般的な取引です。

AgencyTransactionにおいて、ValueAssetIdは通貨のアセット（Currency）でなければならず、AssetIdと同じ値を使用することはできません。購入リストと販売リストは、それぞれ少なくとも1つの注文を格納する必要があります。トランザクションには全く未取引である注文は含めることはできませんが、部分的に取引が実行されている注文であれば可能です。部分的に取引が実行されている注文では、最低価格をつける必要があります。買い注文では顧客が指定した価格より低い値段でトランザクションを実行することができ、反対に売り注文では顧客が指定した価格より高い価格とすることができます。
...
トランザクションと共に注文が転送される場合、すでにアセット、Currency、エージェントなどの情報が含まれているため、次の形式に注文を圧縮できます。
